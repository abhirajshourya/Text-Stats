...
export default function TextForm(props) {
    ...
    const handleCopy = () => {
        var text = document.getElementById('form-text-area');
        navigator.clipboard.writeText(text.value);
        props.alertMaker("Copied!", "Success");
    }
    ...
    return (
        <>
            <div className="container" style={mystyle}>
                <div className="mb-3">
                    <h2>
                        <label htmlFor="form-text-area" className="form-label">
                            {props.label}
                        </label>
                    </h2>
                    <textarea
                ...
                <button disabled={text.length === 0} className="btn-primary btn m-2" onClick={handleCopy}>
                    Copy Text
                </button>
            </div>
            ...
            <div className="container m-2" style={mystyle}>
                ...
                <h3 >Preview</h3>
                <p style={{ border: "1px solid grey", padding: "20px", color: props.mode === 'light' ? 'black' : 'white' }}>{text.length > 0 ? text : "Nothing to preview"}</p>
            </div>
        </>
    );
}
